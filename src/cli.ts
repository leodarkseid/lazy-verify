...
import { SourcifyVerifier, EtherscanVerifier, VerificationProvider } from './verifier.js';
import { heuristicVerification } from './heuristics.js';

const argsOptions = {
    address: { type: 'string' as const, short: 'a' },
    chain: { type: 'string' as const, short: 'c' },
    contract: { type: 'string' as const, short: 'f' },
    'compiler-version': { type: 'string' as const, short: 'v' },
    'verify-proxy': { type: 'boolean' as const, short: 'p', default: false },
    'etherscan-api-key': { type: 'string' as const, short: 'k' },
...
async function main() {
    ...
    const chainId = parseInt(values.chain as string, 10);
    const contractAbsPath = path.resolve(values.contract as string);
    const manualCompilerOverride = values['compiler-version'] as string | undefined;
    const verifyProxy = values['verify-proxy'] as boolean;
    const etherscanKey = values['etherscan-api-key'] as string;
    ...
    await Promise.all(providers.map(async (provider) => {
        ...
        try {
            const hResult = await heuristicVerification(
                provider,
                finalAddressToVerify,
                chainId,
                targetSolcVersion,
                standardJson,
                contractName
